...
#include <iostream> 
 ...
 void pdcp_entity_tx::compute_checksum(uint8_t* data, uint8_t result[2])
 {
 ...
 void pdcp_entity_tx::retransmit_all_pdus()
{
 ...
 bool pdcp_entity_tx::write_data_pdu_header(byte_buffer& buf, const pdcp_data_pdu_header& hdr) const
 {
   ...
   byte_buffer::iterator hdr_writer = buf.begin();// payload of byte_buffer_segment
   ...
   switch (cfg.sn_size) {
     ...
     case pdcp_sn_size::size18bits:
       /*  |   |   |   |   |   |   |   |   | 
           |D/C| R | R | R | R | R |  S-N  | Oct 1 
           |             S-N               | } Oct 2
           |             S-N               | } Oct 3
       */
       *hdr_writer |= (hdr.sn & 0x00030000U) >> 16U; 
       // `0x0003` = 2 bits du haut
       // >>16U = enleve les 16 bits de poids faible = 2 octets `0000`
       hdr_writer++; // +1 octet
       if (hdr_writer == buf.end()) {
         logger.log_error("Not enough space to write header. sn_size={}", cfg.sn_size);
       }
       *hdr_writer = (hdr.sn & 0x0000ff00U) >> 8U;
       // `0x0000ff00U` = 8 bits du milieu
       // >>8U = enleve les 8 bits de poids faible = octet `00`
       hdr_writer++;
       if (hdr_writer == buf.end()) {
         logger.log_error("Not enough space to write header. sn_size={}", cfg.sn_size);
       }
       *hdr_writer = hdr.sn & 0x000000ffU;
       // `0x000000ffU` = 8 bits du bas
       break;
     default:
