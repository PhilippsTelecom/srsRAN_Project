...
#include <cstdint>
 ...
 uint8_t* byte_buffer::get_payload_(size_t start,size_t length)
 {
   // Problem with bounds: return
   if ( (start + length) > ctrl_blk_ptr->pkt_len){
     return nullptr; 
   }
   
   // Go to good node (where to start extraction)
   size_t cursor=0; 
   node_t* seg = ctrl_blk_ptr->segments.head; 
   while ( (seg != nullptr) and (cursor+seg->length()<start) )
   {
 ...
 void byte_buffer::set_payload_(size_t start,uint8_t value)
 {
   // Problem with bounds: return
   if ( start >= ctrl_blk_ptr->pkt_len){
     return ; 
   }
