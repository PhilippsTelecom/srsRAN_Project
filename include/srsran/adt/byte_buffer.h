 ...
 */
 ...
 public:
   ...
   /// Rajout: renvoie une partie du payload
   /// \param[in] start a partir de la ou ca commence
   /// \param[in] length taille de ce qui est renvoye
   uint8_t* get_payload_(size_t start,size_t length);
 
   /// Rajout: modifie un octet du payload (pas de rajout)
   /// \param[in] start ou se trouve l'octet a modifier
   /// \param[in] value les 8 bits a remplacer
   void set_payload_(size_t start,uint8_t value);
